...
TEST(TestTrajectoryGeneration, CreateCircularTrajectory)
{
  ...
  // Initial point
  EXPECT_NEAR(
    trajectory.front().pose.position.x,
    0.0,
    1e-3);

  EXPECT_NEAR(
    trajectory.front().pose.position.y,
    0.0,
    1e-3);


  // Final point of quarter circle
  EXPECT_NEAR(
    trajectory.back().pose.position.x,
    radius,
    1e-3);

  EXPECT_NEAR(
    trajectory.back().pose.position.y,
    radius,
    1e-3);
...
TEST_F(TestSteeringRate4WS, SteeringRateBaseline2WS)
{
  auto trajectory =
    createCircularTrajectory(
      10.0,
